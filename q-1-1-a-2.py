...
class Autoencoder:
    ...
    def save_weights(self):
        w_pickle = open('W_a_2'+str(int(time.time())), 'wb')
...
ae = Autoencoder(len(X_train.columns),[25,20,15,14,15,20,25],'linear','linear','sigmoid')
orig_X = X_train.values


ae.fit(orig_X,1e-2,100)
ae.save_weights()
...
np.savetxt("compressed_intrusion_data_a_2.csv", compressed_data, delimiter=",",fmt="%s")
